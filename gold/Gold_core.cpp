...
Gold_data &Gold_core::ld_data_ref(Inst_id iid) {
  return find_entry(iid).ld_data;
}

Gold_data &Gold_core::st_data_ref(Inst_id iid) {
  return find_entry(iid).st_data;
}
...
const Gold_data &Gold_core::ld_perform(Inst_id iid) {
  ...
  if (rob.back().rid != iid) { // Not if load is the oldest in ROB
    ...
    Rob_queue::reverse_iterator rob_end = std::find_if(rob.rbegin(), rob.rend(), [&iid](const Rob_entry& x) { return x.rid == iid;});
...
void Gold_core::st_locally_merged(Inst_id iid1, Inst_id iid2) {
  ...
  for (; rob_it2!= rob.end(); ++rob_it2) {
    ...
    std::cout
      << "WARNING: between st id:" << iid2
      << "and id:" << iid1
      << " there is a partial overlap st id:" << rob_it2->rid
      << "\n";
    d2.dump("merged");
    rob_it2->dump("match");
  }
...
void Gold_core::st_globally_perform(Inst_id iid) {
  ...
  for(; it!=rob.end(); ++it) {
    ...
    if (it->st_data.has_partial_overlap(rob_it1->st_data)) {
      std::cout
        << "WARNING: older than st id:" << iid
        << " there is a partial overlap id:" << it->rid
        << "\n";
  ...
  if (only_reads) { // try to retire these
    ...
    while(rob.back().rid != iid) {
      ...
      if (!notified && (!oldest.error.empty() || !oldest.performed)) {
        notified = true;
        std::cout << "FAIL: ld:" << oldest.rid
          << " is oldest with error:"
          << oldest.error
          << "\n";
        dump();
      }
